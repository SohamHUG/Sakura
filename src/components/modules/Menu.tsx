...
export default function SakuraMenu() {
    ...
    return (
        <div id="smooth-menu" className="mx-auto w-full h-auto bg-black px-4 py-10 text-neutral-100 overflow-x-hidden ">
            {/* Header */}
            <header className="mb-8 flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
                ...
                <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center">
                    ...
                    <select
                        ...
                        className="rounded-2xl border border-neutral-700 bg-neutral-900/60 px-3 py-2 font-noto"
                    >
                ...
                {(["vegan", "vegetarian", 'vegetarian-option', "gluten-free", "spicy", 'grill', "signature", "new"] as DietaryTag[]).map(
                    (t) => (
                        <button
                            ...
                            className={
                                "rounded-full border px-3 py-1 text-sm transition font-kaisei" +
                                (diet[t]
                ...
                {ALL_CATEGORIES.map((c) =>
                    grouped[c]?.length ? (
                        <section key={c} id={c.replace(/\s+/g, "-").toLowerCase()}>
                            <h2 className="mb-4 text-2xl font-bold tracking-tight font-kaisei">{c}</h2>
